...
export function V2Launchpad({ onBusy, mode = "launch" }: { onBusy: (busy: boolean) => void; mode?: "launch" | "swap" }) {
 ...
 const discovered = useQuery({queryKey:["arc-token-discovery"], enabled:pickerOpen, staleTime:60000, retry:false, queryFn:()=>discoverArcTokens(50)});
 const visibleTokens = useMemo(() => { const query = tokenSearch.trim().toLowerCase(); return (market.data?.tokens ?? []).filter(item => !query || `${item.name} ${item.symbol} ${item.address}`.toLowerCase().includes(query)); }, [market.data?.tokens, tokenSearch]);
 const discoveredTokens = useMemo(() => { const supported = new Set((market.data?.tokens ?? []).map(item => item.address.toLowerCase())); const query = tokenSearch.trim().toLowerCase(); return (discovered.data ?? []).filter(item => !supported.has(item.address.toLowerCase()) && (!query || `${item.name} ${item.symbol} ${item.address}`.toLowerCase().includes(query))); }, [discovered.data, market.data?.tokens, tokenSearch]);
 ...
 async function launch() {
  ...
  setModal(null); setMessage("Token launched. Buy through the curve to reach graduation.");
 }
 async function swap() {
  ...
  {!isAddress(factory) ? <div className="glass-panel v2-empty"><h3>Set up a graduating-token market</h3><p>Legacy coins cannot graduate. Deploy a V2 market or connect an existing one.</p><button className="btn-primary" disabled={busy} onClick={()=>setModal("settings")}>Set up market</button></div> :
   <div className="v2-columns">
    <div>
     ...
     <div className="v2-token-grid">{market.data?.tokens.map(t=><button className="market-card v2-token-card" key={t.address} disabled={busy} aria-label={`Select graduating ${t.name}`} onClick={()=>{setSelected(t.address);setAmount(t.graduated ? "1" : "100");setError("");}}>
    ...
    <div className="glass-panel v2-ticket">
     {mode === "swap" && <>
      <button type="button" role="combobox" aria-label="V2 coin" aria-expanded={pickerOpen} aria-controls="v2-token-options" className="token-picker-trigger" disabled={busy} onClick={()=>setPickerOpen(true)}>
       {token ? <><TokenImage src={token.image} name={token.name} size={28}/><span>{token.name} ({token.symbol})</span></> : <span>Select a token</span>}<span aria-hidden>⌄</span>
      </button>
      <FormDialog open={pickerOpen} onOpenChange={setPickerOpen} busy={busy} title="Select a token" description="Choose a token from the graduating-token market.">
       <div className="token-picker-search"><Search size={15}/><input aria-label="Search V2 tokens" value={tokenSearch} onChange={event=>setTokenSearch(event.target.value)} placeholder="Search by name, ticker, or address"/></div>
       <div id="v2-token-options" className="token-picker-list" role="listbox" aria-label="Available V2 tokens">
        {visibleTokens.map(item=><button role="option" aria-selected={item.address === token?.address} className="token-picker-item" key={item.address} onClick={()=>{setSelected(item.address);setAmount(item.graduated ? "1" : "100");setError("");setTokenSearch("");setPickerOpen(false);}}><TokenImage src={item.image} name={item.name} size={38}/><span><strong>{item.name}</strong><small>{item.symbol} · {item.graduated ? "Pool live" : "Bonding curve"}</small></span><span className="token-picker-arrow">›</span></button>)}
        {discoveredTokens.length > 0 && <p className="token-picker-section">Detected on Arc Testnet</p>}
        {discoveredTokens.map(item=><button role="option" aria-selected="false" aria-disabled="true" className="token-picker-item token-picker-item-disabled" disabled key={item.address} title="This token is detected but has no Mofu curve or pool route"><TokenImage src={item.image} name={item.name} size={38}/><span><strong>{item.name}</strong><small>{item.symbol} · detected · no Mofu route</small></span></button>)}
        {discovered.isError && <p className="token-picker-empty">Arc discovery is unavailable. Graduating tokens are still available.</p>}
        {!visibleTokens.length && !discoveredTokens.length && <p className="token-picker-empty">No tokens match your search.</p>}
       </div>
      </FormDialog>
     </>}
     {!selected ? <p>Select a token to buy, sell, or inspect its pool.</p> : !token ? <p>{detail.isError ? "Token could not be loaded." : "Loading token…"}</p> : <>
